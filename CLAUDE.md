...
- `pnpm typecheck`, `pnpm lint`, `pnpm test` – musí projít před dokončením úkolu
- Závislost do konkrétního balíčku: `pnpm --filter <web|@sport/contracts|@sport/domain> add <pkg>`

## Struktura

- `apps/web` – Nuxt 4 (srcDir `app/`), Tailwind CSS 4 přes `@tailwindcss/vite`, `@nuxtjs/i18n`
  (locale soubory v `apps/web/i18n/locales/`)
- `packages/contracts` – Zod schémata + `z.infer` typy + repository rozhraní
- `packages/domain` – čistá TS logika bez Vue/Nuxt
- Balíčky jsou source-only (`exports` → `src/index.ts`), nic se nebuilduje.

## Konvence

- Data jen přes repository rozhraní; mock data v `apps/web/mocks/`, nikdy natvrdo v komponentách.
  Implementaci vybírá `runtimeConfig.public.dataSource` (`NUXT_PUBLIC_DATA_SOURCE`).
...
- TypeScript je záměrně na 6.0.x (catalog v `pnpm-workspace.yaml`) – typescript-eslint
  zatím TS 7 nepodporuje.
